...
def main():
    if len(sys.argv) < 2:
        ...
        print("\nExamples:")
        print('  uv run adw_plan_build_validate_review.py "Add dark mode toggle"')
        print('  uv run adw_plan_build_validate_review.py "Fix login bug" custom_id')
        sys.exit(1)
    ...
    print(f"{'#' * 60}")
    print(f"# ADW ORCHESTRATOR: Plan + Build + Validate + Review")
    print(f"{'#' * 60}")
    ...
    print(f"\n{'#' * 60}")
    print(f"# WORKFLOW COMPLETED: Plan + Build + Validate + Review")
    print(f"{'#' * 60}")
    print(f"ADW ID: {adw_id}")
