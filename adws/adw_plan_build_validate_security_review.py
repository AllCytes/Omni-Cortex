...
# ///
"""ADW Plan + Build + Validate + Security + Review Orchestrator."""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: uv run adw_plan_build_validate_security_review.py <request> [adw-id]")
        print("\nThis runs: Plan -> Build -> Validate -> Security -> Review")
        print("\nExamples:")
        print('  uv run adw_plan_build_validate_security_review.py "Add dark mode toggle"')
        print('  uv run adw_plan_build_validate_security_review.py "Fix login bug" custom_id')
        sys.exit(1)
    ...
    print(f"{'#' * 60}")
    print(f"# ADW ORCHESTRATOR: Plan + Build + Validate + Security + Review")
    print(f"{'#' * 60}")
    ...
    phases = [
        ...
        ("SECURITY", ["uv", "run", os.path.join(script_dir, "adw_security.py"), adw_id]),
    ...
    print(f"\n{'#' * 60}")
    print(f"# WORKFLOW COMPLETED: Plan + Build + Validate + Security + Review")
    print(f"{'#' * 60}")
    print(f"ADW ID: {adw_id}")
    print(f"State: agents/{adw_id}/adw_state.json")
    print()
    print("Next steps:")
    print(f"  - Retrospective: uv run adws/adw_retrospective.py {adw_id}")
